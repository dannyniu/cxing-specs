...
>   - The expression `a === b`, is true if and only if the value proper of the
>     value native object of both operands compares equal.

**Note**: the actual adopted definition for loose comparison slightly deviates
away from this proposal.
...
- For strict comparision operator `===`:
  ...
  - The transitive property is preserved, but would behave
    erratically for string objects.

Should they employ such concept in the program code, if one were to find out
the 'class hierarchy' , they can call the `equals()` method explicitly to
de-symmetricize.
